...
uint64_t listdir(const char *path, uint64_t index, char *buffer) {
  ...
  // list directory (path pointer in rbx, path length in rcx, index in rdx,
  // return name in r8) copy name to pointer in r8, return name length in rax
  ...
  asm volatile("mov %[syscall], %%rax\n"
               ...
               "int $0x80\n"
               : "=rax"(ret)
               : [syscall] "i"(SYSCALL_LISTDIR), [path] "m"(path),
                 [len] "m"(len), [index] "m"(index), [buffer] "m"(buffer)
               : "rbx", "rcx", "rdx", "r8");
...
uint64_t read(uint64_t fd, void *buffer, uint64_t size) {
  ...
  asm volatile("mov %[syscall], %%rax\n"
               ...
               "int $0x80\n"
               : "=rax"(ret)
               : [syscall] "i"(SYSCALL_READ), [fd] "m"(fd),
